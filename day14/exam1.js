...
let titleDom = document.querySelector("#title"); //특정 마크업 가져오기
titleDom.innerHTML = "DOM 조작 성공!" // 특정 마크업내 텍스트 .innerHTML <마크업> durlrk inner </마크업>
...
function func4(){
    ...
    // DOM객체.classList.toggle() : 만약에 클래스가 존재하면 삭제 /없으면 추가
...
const names = [];
function func9(){
    const nameInputDom = document.querySelector("#nameInput");//입력
    const name =nameInputDom.value;
    names.push(name);//저장

    const namePrintDom = document.querySelector("#namePrint");
    namePrintDom.innerHTML = names
}
